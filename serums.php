...
<head>
    ...
    <title>Sérums Skincare</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <header>
        <h1>Sérums Skincare</h1>
        <nav>
    ...
    <main>
        <section class="product-list">
            ...
            if (($handle = fopen($file, 'r')) !== FALSE) {
                ...
                while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
                    // Vérifier si le type est "Sérum"
                    if ($data[0] == 'Sérum') {
                        echo '<article class="product">';
                        echo '<h3>' . htmlspecialchars($data[1]) . '</h3>';
                        echo '<img src="serum.jpg" alt="' . htmlspecialchars($data[1]) . '">';
        ...
        <section class="add-product">
            <h2>Ajouter un Sérum</h2>
            <form action="add_product.php" method="post">
                <input type="hidden" name="type" value="Sérum">
